...
def get_monthly_analytics(db: Session, user_id: int, month: int, year: int):
    """Total spent + breakdown by category for a given month/year (cash-out only)."""
    q = (
        db.query(
            Expense.category_id,
...
def get_top_category(db: Session, user_id: int, month: int, year: int):
    """Highest-spend category for a given month/year (cash-out only)."""
    q = (
        db.query(
            Expense.category_id,
            func.coalesce(func.sum(Expense.amount), 0).label("total_amount"),
...
def get_trend(db: Session, user_id: int, months: int):
    """Month-by-month spending trend for the last N months (cash-out only)."""
    if months <= 0:
