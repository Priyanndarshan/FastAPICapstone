...
- Uses repositories for DB access and keeps rules in one place.
"""
...
def create_user_expense(db: Session, user_id: int, *, payload: dict):
...
def get_user_expense(db: Session, user_id: int, expense_id: int):
...
def update_user_expense(db: Session, user_id: int, expense_id: int, *, updates: dict):
    expense = get_expense_for_user(db, user_id, expense_id)
    if not expense:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Expense not found")
...
def delete_user_expense(db: Session, user_id: int, expense_id: int):
    expense = get_expense_for_user(db, user_id, expense_id)
    if not expense:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Expense not found")
